...
def random_affine(img, mask, targets, targets2, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2), borderValue=(123.7, 116.3, 103.5), all_bbox=None):
    ...
    imw = cv2.warpPerspective(img, M, dsize=(height, height), flags=cv2.INTER_LINEAR, borderValue=borderValue)  # BGR order borderValue
    if mask is not None:
        maskw = cv2.warpPerspective(mask, M, dsize=(height, height), flags=cv2.INTER_NEAREST, borderValue=255)  # BGR order borderValue
    else:
        maskw = None

    # need to change to two bbox
    # Return warped points also
    if type(targets)==type([1]) and type(targets2)==type([1]):
        targetlist=[]
        targetlist2 = []
        for bbox in targets:
            targetlist.append(wrap_points(bbox, M, height, a))
        for bbox2 in targets2:
            targetlist2.append(wrap_points(bbox2, M, height, a))
        return imw, maskw, targetlist, targets2, M

    elif all_bbox is not None:
        targets = wrap_points(targets, M, height, a)
        targets2 = wrap_points(targets2, M, height, a)
        for ii in range(all_bbox.shape[0]):
            all_bbox[ii,:] = wrap_points(all_bbox[ii,:], M, height, a)
        return imw, maskw, targets, targets2, all_bbox, M

    elif targets is not None and targets2 is not None:   ## previous main
        targets = wrap_points(targets, M, height, a)
        targets2 = wrap_points(targets2, M, height, a)
        return imw, maskw, targets, targets2, M
